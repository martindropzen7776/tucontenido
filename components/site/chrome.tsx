      ...
      <div className="grid gap-[clamp(28px,4vw,60px)] border-b border-bone/15 pb-10 min-[620px]:grid-cols-2 lg:grid-cols-[2fr_1fr_1fr]">
        ...
        <div>
          ...
          <ul>
            {NAV.slice(1, 4).map((n) => (
              <li key={n.href}>
                <a href={n.href} className="tap text-[15px] text-bone/70 hover:text-acid">
                  {n.label}
                </a>
              </li>
            ))}
        ...
        <div>
          ...
          <ul>
            ...
            <li>
              <a href={`mailto:${EMAIL}`} className="tap text-[15px] text-bone/70 hover:text-acid">
                Email
              </a>
            </li>
            {INSTAGRAM && (
...
export function WaFab() {
  return (
    <WaLink
      ...
      className="fixed bottom-4 right-4 z-[900] inline-flex items-center gap-[11px] border-2 border-ink bg-acid px-[15px] py-3.5 text-sm font-semibold text-ink shadow-[5px_5px_0_var(--color-ink)] transition-transform hover:-translate-x-[3px] hover:-translate-y-[3px] sm:bottom-5 sm:right-5 sm:px-[22px]"
    >
